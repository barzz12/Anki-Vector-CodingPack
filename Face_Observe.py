...
def main():
    ...
    def on_robot_observed_face(robot, event_type, event, evt):
        ...
        robot.behavior.say_text("Hey")
        robot.behavior.say_text(event.name)
        robot.behavior.say_text("how are you feeling?")
        time.sleep(1)
        ...
        if event.expression == 2:
            ...
            robot.behavior.say_text("You are making me happy, so let's dance!")
            robot.behavior.set_head_angle(MAX_HEAD_ANGLE)
            # robot.behavior.drive_on_charger()

        elif event.expression == 3:
            robot.anim.play_animation('anim_pounce_success_02')
            robot.behavior.say_text("Oh my! What surprised you? Should I be scared?")
            robot.behavior.set_head_angle(MAX_HEAD_ANGLE)
            # robot.behavior.drive_on_charger()

        elif event.expression == 4:
            robot.behavior.say_text("Are you mad? Let me tell you a joke. Why did the chicken cross the road?")
            time.sleep(2)
            ...
            robot.behavior.say_text("To get to the other side.")
            robot.behavior.set_head_angle(MAX_HEAD_ANGLE)
            # robot.behavior.drive_on_charger()

        elif event.expression == 5:
            robot.anim.play_animation('anim_pounce_success_02')
            robot.behavior.say_text("Cheer up buttercup!")
            ...
            robot.anim.play_animation('anim_pounce_success_02')
            robot.behavior.say_text("Show some expression.")
            robot.behavior.set_head_angle(MAX_HEAD_ANGLE)
            # robot.behavior.drive_on_charger()

        else:
            print("unexpected expression", event.expression)
            robot.behavior.say.text("How are you feeling?")
            robot.anim.play_animation('anim_pounce_success_02')
            robot.behavior.set_head_angle(MAX_HEAD_ANGLE)
            # robot.behavior.drive_on_charger()
    ...
    with anki_vector.Robot(args.serial, enable_face_detection=True, show_viewer=True) as robot:
